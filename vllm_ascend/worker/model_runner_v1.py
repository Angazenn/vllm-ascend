...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Sparse C8 remains a non-offload cache layout. Legacy decode offload
        # always uses the BF16 K/V/indexer tuple.
        self.use_sparse_c8_indexer = self.use_sparse_c8 and not self.use_offload
    ...
    def _prepare_inputs(
        ...
        if self.use_offload:
            ...
            # Prefer the connector's completed CPU-block count. For decode
            # offload this deliberately lags newly staged layerwise saves by one
            # forward, which prevents async speculative metadata from routing a
            # block to CPU before its copy is usable. Remote-prefill connectors
            # report the prefix blocks already present in their CPU pool.
    ...
    def _build_attention_metadata(
        ...
        for kv_cache_gid, kv_cache_group in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            if self.use_sfa_decode_offload and isinstance(
                kv_cache_group.kv_cache_spec, OffloadMLAAttentionSpec
            ):
                # The MTP proposer builds metadata only for its draft main
                # attention layer. Carry the split indexer group's addressing
                # alongside the main group so the draft indexer still writes
                # through its own block table and slot mapping.
                cm.indexer_block_table_tensor = block_table_gid_0
                cm.indexer_slot_mapping = slot_mapping_gid_0
                cm.num_offloaded_blocks = self.num_offloaded_blocks.gpu[
                    :num_reqs
                ]
    ...
    def _merge_kv_cache_tensors_for_layer_reuse(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.use_sfa_layerwise_ascend_store and any(
            isinstance(spec, AscendSFALayerwiseIndexerCacheSpec)
            ...
            pool_names = merge_sfa_physical_pool_owners(
                main_names,
                get_layerwise_storage_indices(len(main_names), extra_config),
                [group.layer_names for group in indexer_groups],
            )
            expected = set(layer_kv_cache_spec)
            actual = [name for pool in pool_names for name in pool]
            if set(actual) != expected or len(actual) != len(expected):
                raise ValueError(
                    "GLM SFA layerwise pool rewrite must cover every cache "
                    "owner exactly once"
                )
            tensor_sizes = {tensor.size for tensor in old_tensors}
            if len(tensor_sizes) != 1:
                raise ValueError(
                    "GLM SFA main and indexer cache pages cannot share "
                    f"physical pools: tensor sizes={sorted(tensor_sizes)}"
                )
            kv_cache_config.kv_cache_tensors = [
                KVCacheTensor(size=old_tensors[0].size, shared_by=owners)
                for owners in pool_names
            ]
            logger.info(
                "GLM SFA layerwise cache reuse: %d cache owners -> %d "
                "physical pools",
                len(expected),
                len(pool_names),
            )
    ...
    def _allocate_kv_cache_tensors(self, kv_cache_config: KVCacheConfig) -> dict[str, torch.Tensor]:
        ...
        for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
            ...
            if any(
                ...
                if not any(
                    isinstance(
                        layer_kv_cache_spec[name], OffloadMLAAttentionSpec
                    )
                    for name in shared_layer_names
                ):
                    raise ValueError(
                        "A direct SFA decode indexer alias must share a "
                        "physical pool with one main-KV owner"
                    )
                # Allocate the shared raw K/V bytes through the main owner.
                # The indexer owner is bound to an alias view during reshape.
                shared_layer_names.sort(
                    key=lambda name: isinstance(
                        layer_kv_cache_spec[name],
                        AscendSFAOffloadIndexerCacheSpec,
                    )
                )
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(
                    ...
                    kernel_block_size = (
                        attn_backend.get_supported_kernel_block_sizes()[0]
                    )
                    assert (
                        current_kv_cache_spec.block_size
                        % kernel_block_size
                    ...
                    indexer_shape = attn_backend.get_kv_cache_shape(
                        num_manager_blocks * blocks_per_manager_block,
                        kernel_block_size,
                        current_kv_cache_spec.num_kv_heads,
                        current_kv_cache_spec.head_size,
                    )
                ...
                # encounter OOM issue
                    ...
                    dsa_k_cache = (
                        ...
                        else raw_k_tensor.view(dtype).view(
                            dsa_k_cache_shape
                        )
                    )
                    ...
                    assert (
                        total_bytes % current_kv_cache_spec.page_size_bytes == 0
                    )
                    num_manager_blocks = (
                        total_bytes // current_kv_cache_spec.page_size_bytes
                    )
                    kernel_block_size = (
                        attn_backend.get_supported_kernel_block_sizes()[0]
                    )
                    assert (
                        current_kv_cache_spec.block_size % kernel_block_size == 0
                    )
                    blocks_per_manager_block = (
            ...
            for layer_name in kv_cache_group_spec.layer_names:
                ...
                if isinstance(layer_kv_cache_spec, AscendSFAIndexerCacheSpec):
                    ...
                    from vllm_ascend.attention.indexer import (
                        AscendSFALayerwiseIndexerBackend,
                    )

                    attn_backend = AscendSFALayerwiseIndexerBackend
                elif isinstance(
                    layer_kv_cache_spec,
                    AscendSFAOffloadIndexerCacheSpec,
                ):
                    ...
                    attn_backend = AscendSFAOffloadIndexerBackend
                else:
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        if (
            self.use_sparse
            and self.use_offload
            and not self.use_sfa_decode_offload
        ):
        ...
        for layer_name, attn_module in attn_layers.items():
            if self.use_sfa_decode_offload and isinstance(
            ...
            elif self.use_sfa_decode_offload and isinstance(
                ...
                if self.use_sfa_layerwise_ascend_store:
                    hf_cfg = self.model_config.hf_text_config
                    ...
                    cache_sparse_c8 = self.ascend_config.is_sparse_c8_layer(
                        layer_name
                    )
