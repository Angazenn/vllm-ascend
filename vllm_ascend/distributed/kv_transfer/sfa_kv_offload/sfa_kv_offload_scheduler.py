...
class SFAKVOffloadlScheduler:
    ...
    def _select_group_block_ids(self, block_ids) -> list[int]:
        if block_ids is None:
            return []
        if isinstance(block_ids, (tuple, list)) and block_ids and isinstance(block_ids[0], list):
            group_id = self.selected_group_id
            if group_id is None:
                group_id = len(block_ids) - 1
            if group_id < 0:
                group_id += len(block_ids)
            if group_id < 0 or group_id >= len(block_ids):
                return []
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> KVConnectorMetadata:
        ...
        for request in scheduler_output.scheduled_new_reqs:
            block_ids_npu = self._select_group_block_ids(request.block_ids)
        ...
        for i, req_id in enumerate(cached_reqs.req_ids):
            # resumed request
            new_block_ids_npu = self._select_group_block_ids(
