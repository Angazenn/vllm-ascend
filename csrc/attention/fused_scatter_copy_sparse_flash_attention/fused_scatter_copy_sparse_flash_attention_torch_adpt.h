...
inline void npu_fused_scatter_copy_sparse_flash_attention(
  ...
  // Registered MemFabric GVA allocations are exposed as non-owning CPU
  // tensors. The offload manager owns registration; ordinary CPU allocations
  // are not valid sources. Keep these addresses instead of staging in HBM.
  for (const at::Tensor* tensor :
