...
def test_twelve_groups_share_four_layer_slots(config, runtime):
    ...
    cache_config = get_deepseek_v41_kv_cache_config(runtime, groups, pool_bytes_per_block(groups) * 10 + 1)
...
def test_shared_slots_isolate_groups_and_recycled_ids(config, runtime):
    ...
    count = len(groups) + 1
    cfg = get_deepseek_v41_kv_cache_config(runtime, groups, pool_bytes_per_block(groups) * count)
